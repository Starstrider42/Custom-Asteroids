...
namespace Starstrider42.CustomAsteroids
{
    ...
    internal class ValueRange : IPersistenceLoad
    {
        ...
        protected const string PROP_FORMAT =
            "(?<prop>rad|soi|sma|per|apo|ecc|inc|(a|l)pe|lan|mn(a|l)0"
        ...
        static readonly Regex ratioDecl = new Regex (
            "Ratio\\(\\s*" + PLANET_PROP + "\\s*,\\s*(?<ratio>[-+.e\\d]+)\\s*\\)",
            RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture);
        /// <summary>Defines the syntax for an Offset declaration.</summary>
        static readonly Regex sumDecl = new Regex (
            "Offset\\(\\s*" + PLANET_PROP + "\\s*,\\s*(?<incr>[-+.e\\d]+)\\s*\\)",
            RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture);
        ...
        /// Warning: Class invariants should not be assumed to hold true prior to calling
        /// <c>parseAll()</c>.
        /// </summary>
        ///
        /// <remarks><c>this.rawMin</c>, <c>this.rawMax</c>, <c>this.rawAvg</c>, and
        /// <c>this.rawStdDev</c> must contain a representation of the desired object value prior
        /// to method invocation.</remarks>
        ///
        /// <exception cref="TypeInitializationException">Thrown if the ConfigNode could not be
        /// interpreted as a set of floating-point values. The program will be in a consistent
        /// state in the event of an exception.</exception>
        ...
        /// <item><description>a string of the format "Ratio(&lt;Planet&gt;.&lt;stat&gt;, &lt;value&gt;)",
        /// where &lt;Planet&gt; is the name of a loaded celestial body, &lt;stat&gt; is one of
        /// (rad, soi, sma, per, apo, ecc, inc, ape, lpe, lan, mna0, mnl0), and &lt;value&gt; is a
        /// string representation of a floating-point number</description></item>
        /// <item><description>a string of the format "Offset(&lt;Planet&gt;.&lt;stat&gt;, &lt;value&gt;)",
        /// where &lt;Planet&gt;, &lt;stat&gt;, and &lt;value&gt; are as above.</description></item>
        /// </list>
        /// </summary>
        ///
        ...
        ///
        /// <exception cref="ArgumentException">Thrown if <c>rawValue</c> could not be interpreted
        /// as a floating-point value. The program state shall be unchanged in the event of an
        /// exception.</exception>
        public static double parseOrbitalElement (string rawValue)
        {
            ...
            if (ratioDecl.Match (rawValue).Groups [0].Success) {
                ...
                if (!double.TryParse (parsed ["ratio"].ToString (), out ratio)) {
                    throw new ArgumentException (
                        Localizer.Format ("#autoLOC_CustomAsteroids_ErrorNoFloat",
                                          rawValue, parsed ["ratio"]));
                }
                retVal = getPlanetProperty (parsed ["planet"].ToString (),
                                            parsed ["prop"].ToString ()) * ratio;
                ...
                if (!double.TryParse (parsed ["incr"].ToString (), out delta)) {
                    throw new ArgumentException (
                        Localizer.Format ("#autoLOC_CustomAsteroids_ErrorNoFloat", rawValue,
                                          parsed ["incr"]));
                }
                retVal = getPlanetProperty (parsed ["planet"].ToString (),
                                            parsed ["prop"].ToString ()) + delta;
            } else if (!double.TryParse (rawValue, out retVal)) {
                throw new ArgumentException (
                    Localizer.Format ("#autoLOC_CustomAsteroids_ErrorNoFloat2", rawValue),
                    nameof (rawValue));
            }
            return retVal;
        }
        ...
        /// </summary>
        ///
        /// <param name="anom">The angle between the periapsis point and a position in the planet's
        /// orbital plane. May be mean, eccentric, or true anomaly.</param>
        ...
        /// <param name="lAn">The longitude of ascending node of this planet's orbital
        /// plane.</param>
        ///
        /// <returns>The angle between the reference direction (coordinate x-axis) and the
        /// projection of a position onto the x-y plane. Will be mean, eccentric, or true longitude,
        /// corresponding to the type of anomaly provided.</returns>
        static double anomalyToLong (double anom, double i, double aPe, double lAn)
        {
            ...
            double sin = Math.Cos (iRad) * Math.Sin (anRad + aPeRad)
                             / Math.Sqrt (cosOnly * cosOnly + sincos * sincos);
            return 180.0 / Math.PI * (Math.Atan2 (sin, cos) + lAnRad);
        }

        /// <summary>Defines the type of probability distribution from which the value is
        /// drawn.</summary>
