...
namespace Starstrider42.CustomAsteroids {
	/// <summary>
	/// Represents a simple combination of boolean conditions. For persistance purposes, the collection can be 
	/// viewed as a collection of strings, with each string representing an elementary condition, plus additional 
	/// data on how the conditions are related.
	/// </summary>
	internal class Condition {
		/// <summary>Determines whether all conditions or any conditions must be met.</summary>
		[Persistent] private Operator combine;

		// TODO: find a way to allow collection classes to contain ConfigNode values that don't represent elements
		[Persistent(collectionIndex = "condition")]
		private readonly ConditionGroup<string> conditions;
		...
		public Condition() {
			this.combine = Operator.And;
			this.conditions = new ConditionGroup<string>();
		}
	...
	/// <summary>
	/// Represents a simple combination of boolean conditions. For persistance purposes, the collection can be 
	/// viewed as a collection of strings, with each string representing an elementary condition.
	/// </summary>
	/// 
	/// <remarks>To be recognized as a collection by ConfigNodes, a class must implement 
	/// <c>System.Collections.Generic.IEnumerable<T></c>, have a matching type parameter <c>T</c>, and implement 
	/// <c>System.Collections.IList</c>. Quite the odd combination...</remarks>
	/// 
	/// <typeparam name="Dummy">Required for proper ConfigNode handling. Only Condition&lt;string&rt; will 
	...
	internal class ConditionGroup<Dummy> : System.Collections.IList, IEnumerable<string> {
		...
		private readonly IList<GamePredicate> clauses;
		...
		public ConditionGroup() {
			this.clauses = new List<GamePredicate>(2);
		}
		...
		private static string unparse(GamePredicate innerData) {
			try {
		...
		private interface GamePredicate {
			/// <summary>
			/// Evaluates this predicate.
			/// </summary>
			/// 
			/// <exception cref="System.InvalidOperationException">Thrown if the condition is corrupted or otherwise 
			/// impossible to evaluate.</exception>
			bool check();
			...
			/// 
			/// <exception cref="InvalidOperationException">Thrown if this predicate cannot be parsed from any 
			/// string.</exception>
			string ToString();
		}
		...
		/// </summary>
		private class AchievementPredicate : GamePredicate {
			/// <summary>A set of paths representing the location of the achievement within the progress tree.</summary>
			private readonly IList<string[]> lookup;
			/// <summary>Allows testing of specifically manned or unmanned missions.</summary>
			private readonly VesselType whoQualifies;

			/// <summary>
			/// Wraps the given achievement as a predicate test.
			/// </summary>
			/// 
			/// <param name="paths">A collection of paths. Each path gives, in heirarchical order, the nested locations 
			/// of the ProgressNode to test. If more than one path is given, any of the corresponding achievements 
			/// may be used to satisfy this predicate.</param>
			...
			internal AchievementPredicate(ICollection<string[]> paths, VesselType qualifies = VesselType.Any) {
				this.lookup = new List<string[]>(paths);
				this.whoQualifies = qualifies;
			}
		...
		/// </summary>
		private class VesselPredicate : GamePredicate {
			/// <summary>
			/// The index associated with the planet of interest. Assumed to remain constant across scene changes.
			/// </summary>
			private readonly int body;

			/// <summary>The state in which a vessel must be present around body.</summary>
			private readonly Test state;
			...
			/// </summary>
			/// 
			/// <param name="body">The name of the celestial body where vessels are to be found.</param>
			/// <param name="condition">What vessels must be doing to pass the test.</param>
			/// <param name="type">Whether the predicate can be satisfied by manned vessels, unmanned 
			/// vessels, or both.</param>
			internal VesselPredicate(string body, Test condition, VesselType type) {
				...
				this.whoQualifies = type;
			}

			public bool check() {
				foreach (Vessel v in FlightGlobals.Vessels) {
					if (v.vesselType == global::VesselType.SpaceObject || v.vesselType == global::VesselType.Unknown) {
						continue;
					}
		...
		private class StringView : IEnumerator<String> {
			private readonly IEnumerator<GamePredicate> baseEnum;
