...
namespace Starstrider42.CustomAsteroids
{
    ...
    internal class SizeRange : ValueRange
    {
        ...
        static readonly Regex mmrDecl = new Regex (
            "Resonance\\(\\s*" + PLANET_FORMAT + "\\s*,\\s*(?<m>\\d+)\\s*:\\s*(?<n>\\d+)\\s*\\)",
        ...
        /// <summary>
        /// Converts an arbitrary string representation of an orbit size to a specific value. The
        /// string must have one of the following formats:
        /// <list type="bullet">
        ...
        /// vesc, vorb, vmin, vmax), and &lt;value&gt; is a string representation of a
        /// floating-point number</description></item>
        ...
        /// <item><description>a string of the format "Resonance(&lt;Planet&gt;, &lt;m&gt;:&lt;n&gt;)",
        /// where &lt;Planet&gt; is the name of a loaded celestial body, and &lt;m&gt; and &lt;n&gt;
        /// are string representations of positive integers. In keeping with standard astronomical
        /// convention, m &gt; n means an orbit inside that of &lt;Planet&gt;, while m &lt; n means
        /// an exterior orbit</description></item>
        /// </list>
        /// </summary>
        ///
        ...
        protected static double parseOrbitSize (string rawValue)
        {
            ...
            if (parsed [0].Success) {
                int m, n;
                if (!int.TryParse (parsed ["m"].ToString (), out m)) {
                    throw new ArgumentException (
                        Localizer.Format ("#autoLOC_CustomAsteroids_ErrorNoInteger",
                                          rawValue, parsed ["m"]));
                }
                if (!int.TryParse (parsed ["n"].ToString (), out n)) {
                    throw new ArgumentException (
                        Localizer.Format ("#autoLOC_CustomAsteroids_ErrorNoInteger",
                                          rawValue, parsed ["n"]));
                }
                if (m <= 0 || n <= 0) {
                    throw new ArgumentException (
                        Localizer.Format ("#autoLOC_CustomAsteroids_ErrorMmrNotPositive",
                                          m, n, rawValue));
                }

                return getPlanetProperty (parsed ["planet"].ToString (), "sma")
                    * Math.Pow ((double)n / m, 2.0 / 3.0);
            }
