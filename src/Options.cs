...
namespace Starstrider42.CustomAsteroids
{
    ...
    /// <summary>
    /// Stores a set of configuration options for Custom Asteroids. ConfigNodes are used to manage
    /// option persistence.
    /// </summary>
    internal class Options
    {
        ...
        /// <summary>
        /// Stores current Custom Asteroids options in a config file. The version is stored as well.
        ...
        internal void save ()
        {
            ...
            try {
                ...
                ConfigNode allData = parentNode.AddNode ("CustomAsteroidSettings");
                ...
                Debug.Log ("[CustomAsteroids]: "
                           + Localizer.Format ("#autoLOC_CustomAsteroids_LogOptionsSave"));
            } finally {
        ...
        /// <summary>
        /// Factory method obtaining Custom Asteroids settings from a config file. Will not throw
        /// exceptions.
        /// </summary>
        /// <returns>A newly constructed Options object containing up-to-date settings from the
        /// Custom Asteroids config file, or the default settings if no such file exists or the
        /// file is corrupted.</returns>
        internal static Options load ()
        {
            try {
                Debug.Log ("[CustomAsteroids]: "
                           + Localizer.Format ("#autoLOC_CustomAsteroids_LogOptionsLoad1"));

                Options allOptions = loadNewStyleOptions ();
                if (allOptions == null) {
                ...
                Debug.Log ("[CustomAsteroids]: "
                           + Localizer.Format ("#autoLOC_CustomAsteroids_LogOptionsLoad2"));

                return allOptions;
            } catch (ArgumentException e) {
                Debug.LogError ("[CustomAsteroids]: "
                                + Localizer.Format ("#autoLOC_CustomAsteroids_LogOptionsNoLoad"));
                Debug.LogException (e);
                ScreenMessages.PostScreenMessage (
                    "[CustomAsteroids]: "
                        + Localizer.Format ("#autoLOC_CustomAsteroids_ErrorBasic",
                                            "#autoLOC_CustomAsteroids_LogOptionsNoLoad",
                                            e.Message),
        ...
        /// </summary>
        /// <returns>The options stored in the game databse, or <c>null</c> if none were
        /// found.</returns>
        /// <exception cref="ArgumentException">Thrown if there is a syntax error in one of the
        /// options.</exception>
        static Options loadNewStyleOptions ()
        {
            UrlDir.UrlConfig [] configList =
                GameDatabase.Instance.GetConfigs ("CustomAsteroidSettings");
        ...
        static Options loadOldStyleOptions ()
        {
            // Start with the default options
            var options = new Options ();
        ...
        /// <summary>
        /// Replaces a missing or out-of-date preferences file. Failures to write the file to disk
        /// are logged and ignored.
        /// </summary>
        /// <param name="oldData">The data to store in the new file.</param>
        static void updateOptionFile (Options oldData)
        {
            try {
                oldData.save ();
                if (oldData.versionNumber.Length == 0) {
                    Debug.Log ("[CustomAsteroids]: "
                               + Localizer.Format ("#autoLOC_CustomAsteroids_LogNoOptions",
                                                   optionFile ()));
                } else {
                    Debug.Log ("[CustomAsteroids]: "
                ...
                Debug.LogError ("[CustomAsteroids]: "
                                + Localizer.Format ("#autoLOC_CustomAsteroids_LogOptionsNoSave"));
                Debug.LogException (e);
            }
        }

        /// <summary>
        /// Returns whether or not asteroids may be renamed by their population. Does not throw
        /// exceptions.
        /// </summary>
        ...
        /// <summary>
        /// Returns whether or not asteroid spawning errors should appear in the game. Does not
        /// throw exceptions.
        /// </summary>
        /// <returns><c>true</c> if errors should be put on screen, <c>false</c> if logged
        /// only.</returns>
        ...
        /// <summary>
        /// Returns the time range in which untracked asteroids will disappear. The program state
        /// is unchanged in the event of an exception.
        /// </summary>
        /// <returns>The minimum (<c>first</c>) and maximum (<c>second</c>) number of days an
        /// asteroid can go untracked.</returns>
        ...
        internal Pair<float, float> getUntrackedTimes ()
        {
            if (minUntrackedLifetime < 0.0f) {
                throw new InvalidOperationException (
                    Localizer.Format ("#autoLOC_CustomAsteroids_ErrorOptionsBadMin",
                                      minUntrackedLifetime));
            }
            if (maxUntrackedLifetime <= 0.0f) {
                throw new InvalidOperationException (
                    Localizer.Format ("#autoLOC_CustomAsteroids_ErrorOptionsBadMax",
        ...
        /// <summary>
        /// Identifies the version 1.4 and earlier Custom Asteroids config file. Does not throw
        /// exceptions.
        /// </summary>
        /// <returns>An absolute path to the config file.</returns>
        static string oldOptionFile ()
        {
            return KSPUtil.ApplicationRootPath
                          + "GameData/CustomAsteroids/PluginData/Custom Asteroids Settings.cfg";
        }
        ...
        {
            return KSPUtil.ApplicationRootPath
                          + "GameData/CustomAsteroids/Custom Asteroids Settings.cfg";
        }
